import 'package:flutter/material.dart';

class AppColors {
  AppColors._();

  // Primary
  static const purple = Color(0xFF6C5CE7);
  static const green = Color(0xFF00D9A3);
  static const blue = Color(0xFF4A90E2);

  // Background
  static const darkBgPrimary = Color(0xFF141432);
  static const darkBgSeccondry = Color(0xFF1D1E42);
  static const darkBgTertiary = Color(0xFF28264F);
  static const cardBg = Color(0xFF252541);
  static const inputBg = Color(0xFF2D2D44);

  // Text
  static const textPrimary = Color(0xFFFFFFFF);
  static const textSecondary = Color(0xFF9CA3AF);
  static const textHint = Color(0xFF6B7280);

  // Status
  static const success = Color(0xFF10B981);
  static const error = Color(0xFFEF4444);
  static const warning = Color(0xFFF59E0B);
  static const black = Color(0xFF000000);
  static const white = Color(0xFFFFFFFF);

  /// Password-strength ramp, weakest to strongest. Indexed by
  /// `PasswordStrength.index`, so the order here is load-bearing.
  static const strengthColors = <Color>[error, warning, blue, green];

  /// Divider/hairline between list rows.
  static const divider = Color(0xFF2D2D44);
}
